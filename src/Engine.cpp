...
void Engine::nextLevel() {
	...
	// Extract the map creation and actor initialization code into a separate function
    initializeNewLevel();
...
void Engine::initializeNewLevel() {
    delete map;
    // delete all actors except player and stairs
    for (Actor **it = actors.begin(); it != actors.end(); it++) {
    	if (*it != player && *it != stairs) {
    		delete *it;
    		it = actors.remove(it);
